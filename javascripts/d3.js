  ...
  "hpcc/ESPWorkunit"
    ...
    var DojoD3Graph = declare(DojoD3, {
        ...
        CoOccurrence: function (_target) {
            ...
            var svg = d3.select(target.domNode).append("svg").attr("width", target.width).attr("height", target.height + margin.top + margin.bottom).style("margin-left", 0 + "px").append("g").style("font", "10px sans-serif").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
            var matrix = [];
